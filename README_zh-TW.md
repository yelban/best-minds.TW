...
三人以上可用平行 subagent 跑兩階段（獨立陳述 → 引言複核 → 交鋒 → 收斂），各聲部彼此看不到對方，避免互相錨定。在 Claude Code 上優先走 **Workflow tool**——parallel `agent()` 讓隔離成為結構保證而非指令自律、schema 強制每個立場輸出可比對的格式、流程不可被靜默跳步；備援用 **Agent tool**，交鋒輪以 SendMessage 延續**同一批**聲部 agents，讓每個人物保留第一輪建立的推理脈絡。其中第 1、3 步與盲區掃描借鏡自 Stanford STORM 正源（見下方源流）。

## v2.4.0 更新重點

來自首次完整實戰圓桌的補強——我們把這個 skill 用在它自己的改版上，完整辯論記錄見 [docs/2026-07-07-roundtable-eval-methodology.md](docs/2026-07-07-roundtable-eval-methodology.md)：

- **引言對抗性複核** — subagent 流程中，每聲部配一個複核 agent 實際抓取引用來源、逐條確認標為 verified 的主張，查無實據就降級；聲部自己標 verified 不再算數。
- **圈外聲部條款** — 全體人選同屬一個學科時，必須補一位圈外利害關係人聲部。實戰觀察：三位評測方法學家的圓桌，沒有人質疑「量的東西值不值得量」——組成本身保證了這個盲點。
- **辯論記錄歸檔** — 收斂後把完整辯論（含證據標註與來源的主張、交叉質詢、盲區掃描、綜合裁決）存成日期標記的 markdown 檔，供事後查核引言。
